...
template<typename T> struct delete_object{
    void operator()(T* item) { delete item; }
};

template<typename T> struct delete_pair_second{
...
public:
    ...
    virtual bool find_constant(perm_string by_name,
                               const VType*&typ, Expression*&exp) const;
    ...
    enum transfer_type_t {
        SIGNALS = 1, VARIABLES = 2,
        COMPONENTS = 4, ALL = 0xffff
    };
